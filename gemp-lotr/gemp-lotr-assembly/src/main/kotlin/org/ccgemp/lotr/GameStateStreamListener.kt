...
class GameStateStreamListener(
    ...
    private fun getCardIds(cards: Collection<PhysicalCard>): IntArray {
        return cards.map { it.cardId }.toIntArray()
    }
    ...
    override fun cardCreated(card: PhysicalCard) {
        ...
        if (card.zone.isPublic || publicDiscard || (card.zone.isVisibleByOwner && card.owner == playerId)) appendEvent(
    ...
    override fun cardCreated(card: PhysicalCard, overridePlayerVisibility: Boolean) {
        val publicDiscard = card.zone == Zone.DISCARD && format.discardPileIsPublic()
        if (card.zone.isPublic || publicDiscard || ((overridePlayerVisibility || card.zone.isVisibleByOwner) && (card.owner == playerId))) appendEvent(
            GameEvent(GameEvent.Type.PUT_CARD_INTO_PLAY).card(card)
        )
    }
    ...
    override fun cardsRemoved(playerPerforming: String?, cards: Collection<PhysicalCard>) {
        ...
        for (card in cards) {
            val publicDiscard = card.zone == Zone.DISCARD && format.discardPileIsPublic()
            if (card.zone.isPublic || publicDiscard || (card.zone.isVisibleByOwner && card.owner == playerId)) removedCardsVisibleByPlayer.add(
                card
            )
        }
        if (removedCardsVisibleByPlayer.size > 0) appendEvent(
            GameEvent(GameEvent.Type.REMOVE_CARD_FROM_PLAY).otherCardIds(
    ...
    override fun getAssignedPlayerId(): String {
        return playerId
    }
    ...
    override fun cardAffectedByCard(
        ...
        appendEvent(
            GameEvent(GameEvent.Type.CARD_AFFECTED_BY_CARD).card(card).participantId(playerPerforming)
    ...
    override fun decisionRequired(playerId: String, decision: AwaitingDecision?) {
        if (playerId == this.playerId) appendEvent(
