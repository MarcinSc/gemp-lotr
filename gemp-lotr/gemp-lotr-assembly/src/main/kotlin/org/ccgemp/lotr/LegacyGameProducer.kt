...
class LegacyGameProducer : GameProducer {
    ...
    override fun createGame(gameParticipants: Array<GameParticipant>, gameSettings: GameSettings): Game {
        ...
        val decks = gameParticipants.associateTo(mutableMapOf()) { player ->
            player.playerId to player.playerDeck.toLotroDeck()
        }
        val game = DefaultLotroGame(
            format, decks, userFeedback, legacyObjectsProvider.cardLibrary, gameSettings.timeSettings.toString(),
            !gameSettings.private, null
        )
