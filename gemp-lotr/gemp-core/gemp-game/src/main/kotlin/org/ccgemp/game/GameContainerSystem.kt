...
class GameContainerSystem : GameContainerInterface<Any>, LifecycleObserver, UpdatedSystem {
    ...
    override fun joinGame(gameId: String, playerId: String, admin: Boolean, gameStream: GameStream<Any>): Runnable? {
    ...
    private fun findGameContainer(gameId: String): GameContainer? {
        val container = gameContainers.firstOrNull {
            it.games.contains(gameId)
        }
        return container
    }

    private fun findBestContainer(): GameContainer {
        return gameContainers.minBy { it.games.size }
    }
    ...
    private fun joinGameInGameThread(
        ...
        channelId: String, gameStream: GameStream<Any>
    ) {
